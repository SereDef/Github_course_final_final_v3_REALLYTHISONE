...
3.  [**Existing project - `GitHub` last**]{.darkblue}: the "*proper*" way to connect existing local work to `GitHub`, when there’s already a `Git` history.
...
[The open-source community is cozy, and you are always welcome 😄]{.m-space}

[POV: you want to get a copy of someone else’s repo (`OWNER/REPO`) and propose a change.]{.m-space} 
...
> Assuming you’re already on the source repo page: click on [Fork]{.grey-button}  (upper right corner) > accept the defaults > click on [Create fork]{.green-button}
...
```
This will create a local branch called `BRANCH-NAME` and "switch to it" (or *“check it out”*). 
